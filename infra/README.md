...
`taskveil-realtime-<environment>` Worker service / versionとsecret bindingsをtrafficなしで
用意する。Custom Domainをversion uploadのCLI optionへ混在させない。
...
同じdeployment moduleは`email.<environment>.<base-domain>`のWorker Custom Domain、
auth-email command Queue / DLQ（retention 1時間）、consumer、outbox dispatcher
scheduleも管理する。初回applyより前に、対応する
`taskveil-auth-email-<environment>` Worker service / versionとsecret bindingsを
trafficなしで用意する。

`${project}-${environment}/runtime` secret valueには、選択環境のDB / billing / realtime設定に加え、`TASKVEIL_RESYNC_TOKEN_KEY_CURRENT_ID`と32-byte standard-base64 `TASKVEIL_RESYNC_TOKEN_KEY_CURRENT`を必ず投入する。email verification用にはcurrent / previous OTP HMAC key、current / previous AWS-only state AEAD key、auth-email Worker ingress URL、delivery signing key ID / key、Worker共有delivery payload AEAD key、dispatcher trigger keyを投入する。overlap rotation中だけ対応するprevious keyを保持し、用途ごとの最大token / ciphertext寿命とclock marginを経過してから削除する。異なる用途に同一鍵値を再利用しない。値はOpenTofu variable、plan、stateへ渡さず、Secrets Managerへout-of-band投入する。

EventBridge API Destination connection `${project}-${environment}-email-dispatch`は初回apply前に人間が作成し、`Authorization: Bearer <dispatcher trigger key>`を設定する。deployment moduleは既存connectionをdata sourceとして参照するため、認証値をOpenTofu stateへ保存しない。WorkerのEmail Sending / Queue secrets、Workers Paid契約、sender allowlist、SPF / DKIM / DMARC、Email Preview無効化、data locationはADR-028のrelease gateを通過するまでtrafficを有効化しない。
